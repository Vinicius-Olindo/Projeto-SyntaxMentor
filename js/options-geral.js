...
if (document.body.classList.contains('geral-page')) {
    ...
    function configurarTTS() {
        const container = document.querySelector('.card-aparencia');
        if (!container) return;
        
        const ttsHtml = `
            <div class="config-item" style="margin-top: 16px; padding-top: 16px; border-top: 1px solid #e5e7eb;">
                <div class="info-text">
                    <label>🔊 Leitura em Voz Alta</label>
                    <p class="desc">Ativar controles de leitura no painel da extensão.</p>
                </div>
                <label class="switch"><input type="checkbox" id="enableTTS"><span class="slider"></span></label>
            </div>
            <div class="config-item-vertical" style="margin-top: 12px;" id="tts-settings" style="display:none;">
                <label style="margin-bottom: 8px;">🎤 Voz padrão</label>
                <select id="tts-voice-select" class="select-box" style="width:100%">
                    <option value="">Carregando vozes...</option>
                </select>
                <label style="margin-top: 12px; margin-bottom: 8px;">⚡ Velocidade da leitura</label>
                <div style="display:flex; align-items:center; gap:10px;">
                    <span>🐢</span>
                    <input type="range" id="tts-speed" min="0.5" max="2.0" step="0.1" value="1.0" style="flex:1">
                    <span>🐇</span>
                    <span id="tts-speed-value" style="min-width:35px">1.0x</span>
                </div>
            </div>
        `;
        
        container.insertAdjacentHTML('beforeend', ttsHtml);
        ...
        chrome.storage.local.get({ enableTTS: true, ttsSpeed: 1.0, ttsVoice: '' }, (res) => {
            if (enableTTS) enableTTS.checked = res.enableTTS;
            if (speedSlider) speedSlider.value = res.ttsSpeed;
            if (speedValue) speedValue.textContent = res.ttsSpeed + 'x';
            if (ttsSettings) ttsSettings.style.display = res.enableTTS ? 'block' : 'none';
            ...
            if (voiceSelect && window.speechSynthesis) {
                setTimeout(() => {
                    ...
                    vozes.forEach(voz => {
                        ...
                        if (res.ttsVoice === voz.name) option.selected = true;
        ...
        if (enableTTS) {
            enableTTS.addEventListener('change', (e) => {
                ...
                if (ttsSettings) ttsSettings.style.display = e.target.checked ? 'block' : 'none';
            });
        }
        
        if (voiceSelect) {
            voiceSelect.addEventListener('change', (e) => {
                chrome.storage.local.set({ ttsVoice: e.target.value });
                if (window.smTTS) window.smTTS.setVoiceByName(e.target.value);
            });
        }
        ...
        if (speedSlider) {
            speedSlider.addEventListener('input', (e) => {
                ...
                chrome.storage.local.set({ ttsSpeed: val });
                if (window.smTTS) window.smTTS.setSpeed(val);
            });
        }
    }

    // Chamar no iniciarPaginaGeral()
    configurarTTS();
