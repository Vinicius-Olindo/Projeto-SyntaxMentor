...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (elCloudSync) elCloudSync.addEventListener('change', (e) => {
        chrome.storage.local.set({ cloudSync: e.target.checked }, () => {
            atualizarStatusSeguranca();
            chrome.storage.local.get({ totalCorrigidas: 0, totalAceitas: 0, totalRecusadas: 0, dicionario_pessoal: [], language: 'pt-BR', erroMaisComum: {}, cloudSync: e.target.checked }, carregarDashboard);
        });
    });
    ...
    if (btnAddBlacklist) {
        btnAddBlacklist.addEventListener('click', () => {
            ...
            if (!currentBlacklist.includes(domain)) {
                ...
                chrome.storage.local.set({ blacklist: currentBlacklist }, atualizarStatusGeral);
            } else {
    ...
    if (btnAddDictionary) {
        btnAddDictionary.addEventListener('click', () => {
            ...
            if (!currentDictionary.includes(word)) {
                ...
                chrome.storage.local.set({ dicionario_pessoal: currentDictionary }, atualizarStatusGeral);
            } else {
    ...
    document.addEventListener('click', (e) => {
        if (recordingTarget && activeBtn && e.target !== activeBtn) cancelarGravacao();
    });
                ...
                try {
                    ...
                    chrome.storage.local.get(['dicionario_pessoal', 'blacklist', 'language'], (res) => {
                        const dicFinal = [...(res.dicionario_pessoal || [])],
                            blackFinal = [...(res.blacklist || [])];
                        let novasPalavras = 0,
                            duplicadasPalavras = 0,
                            novosSites = 0,
                            duplicadosSites = 0;
    ...
    function renderizarModoLeitura() {
        ...
        modoLeituraUl.querySelectorAll('.btn-remove').forEach(btn => btn.addEventListener('click', (e) => {
    ...
    function renderizarWhitelist() {
        ...
        whitelistUl.querySelectorAll('.btn-remove').forEach(btn => btn.addEventListener('click', (e) => {
        ...
        if (statCorrigidasHoje) {
            ...
            chrome.storage.local.get({ correcoesHoje: {}, dataUltimaCorrecao: '' }, (r) => {
                ...
                while (true) {
                    ...
                    } else if (chave === hoje) { data.setDate(data.getDate() - 1); } else break;
                }
